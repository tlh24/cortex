...
def hebb_update(w_, inp, outp, outpavg, lr):
	...
	scale3 = torch.clamp(outpavg, 0.7, 1e6)
...
hidsize = 20
outsize = 4
w_gen1 = torch.mul(torch.randn(insize, hidsize), 1.0/math.sqrt(2.0 * (insize/8.0))); 
w_gen2 = torch.mul(torch.randn(hidsize, outsize), 1.0/math.sqrt(2.0 * (hidsize/8.0))); 
for i in range(0):
...
for k in range(1): 
	...
	w_f3 = torch.mul(torch.randn(outsize, hidsize), math.sqrt(0.2/hidsize))
	...
	for i in range(3000):
		...
		for k in range(5):
			...
			l2u = l2e - l2i - l2li + (torch.randn(hidsize) * 0.01)
			l2a = l2a * 0.99 + l2u * 0.01
		...
		w_f2 = hebb_update(w_f2, l1u, l2u, l2a, 0.002)
		w_l2i = inhib_update(w_l2i, l2u, l2li, 0.002)
		w_b2 = hebb_update(w_b2, l2u5, l1u, torch.ones(l1u.size(0)), 0.008)
		
		w_f3 = hebb_update(w_f3, l2u, l3u, l3a, 0.002)
		w_l3i = inhib_update(w_l3i, l3u, l3li, 0.002)
		w_b3 = hebb_update(w_b3, l3u5, l2u, torch.ones(l2u.size(0)), 0.008)

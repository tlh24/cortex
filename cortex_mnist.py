...
def hebb_update(w_, inp, outp, outpavg, lr):
	...
	w_ = torch.mul(w_, dw)
	# w_ = clamp(w_, -0.02, 1.0) # careful about this -- reverse weights need to be large-2.0, 2.0
...
lr = 0.01 # if the learning rate is too high, it goes chaoitc
for k in range(25): 
	...
	for i in range(N): 
		...
		if (i % 3500) >= 3491: 
			...
			plot_tensor(1, 0, l1i, 'l1i', 0.0, 2.0)
